...
func SearchNodes(decl ast.Node, foundNodes *[]*FoundNodes, parents []ast.Node, searchFunction func(node ast.Node, parents []ast.Node, completed *bool) bool, completed *bool) {
	...
	case *ast.Field:
		...
		if decl.(*ast.Field).Names != nil {
			for _, name := range decl.(*ast.Field).Names {
				SearchNodes(name, foundNodes, parents, searchFunction, completed)
			}
		}
	...
	case *ast.FieldList:
		...
		if decl.(*ast.FieldList).List != nil {
			for _, field := range decl.(*ast.FieldList).List {
				SearchNodes(field, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.BadExpr:
	...
	case *ast.CompositeLit:
		...
		if decl.(*ast.CompositeLit).Elts != nil {
			for _, elt := range decl.(*ast.CompositeLit).Elts {
				SearchNodes(elt, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.ParenExpr:
	...
	case *ast.IndexListExpr:
		...
		if decl.(*ast.IndexListExpr).Indices != nil {
			for _, index := range decl.(*ast.IndexListExpr).Indices {
				SearchNodes(index, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.SliceExpr:
	...
	case *ast.CallExpr:
		...
		if decl.(*ast.CallExpr).Args != nil {
			for _, arg := range decl.(*ast.CallExpr).Args {
				SearchNodes(arg, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.StarExpr:
	...
	case *ast.AssignStmt:
		...
		if decl.(*ast.AssignStmt).Rhs != nil {
			for _, rh := range decl.(*ast.AssignStmt).Rhs {
				SearchNodes(rh, foundNodes, parents, searchFunction, completed)
			}
		}
		if decl.(*ast.AssignStmt).Lhs != nil {
			for _, lh := range decl.(*ast.AssignStmt).Lhs {
				SearchNodes(lh, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.GoStmt:
	...
	case *ast.ReturnStmt:
		...
		if decl.(*ast.ReturnStmt).Results != nil {
			for _, result := range decl.(*ast.ReturnStmt).Results {
				SearchNodes(result, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.BranchStmt:
	...
	case *ast.BlockStmt:
		...
		if decl.(*ast.BlockStmt).List != nil {
			for _, stmt := range decl.(*ast.BlockStmt).List {
				SearchNodes(stmt, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.IfStmt:
	...
	case *ast.CaseClause:
		...
		if decl.(*ast.CaseClause).List != nil {
			for _, expr := range decl.(*ast.CaseClause).List {
				SearchNodes(expr, foundNodes, parents, searchFunction, completed)
			}
		}
		if decl.(*ast.CaseClause).Body != nil {
			for _, stmt := range decl.(*ast.CaseClause).Body {
				SearchNodes(stmt, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.SwitchStmt:
	...
	case *ast.CommClause:
		...
		if decl.(*ast.CommClause).Body != nil {
			for _, stmt := range decl.(*ast.CommClause).Body {
				SearchNodes(stmt, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.SelectStmt:
	...
	case *ast.ValueSpec:
		...
		if decl.(*ast.ValueSpec).Names != nil {
			for _, name := range decl.(*ast.ValueSpec).Names {
				SearchNodes(name, foundNodes, parents, searchFunction, completed)
			}
		}
		SearchNodes(decl.(*ast.ValueSpec).Type, foundNodes, parents, searchFunction, completed)
		if decl.(*ast.ValueSpec).Values != nil {
			for _, value := range decl.(*ast.ValueSpec).Values {
				SearchNodes(value, foundNodes, parents, searchFunction, completed)
			}
		}
	...
	case *ast.GenDecl:
		...
		if decl.(*ast.GenDecl).Specs != nil {
			for _, spec := range decl.(*ast.GenDecl).Specs {
				SearchNodes(spec, foundNodes, parents, searchFunction, completed)
			}
		}
	case *ast.FuncDecl:
	...
	case *ast.File:
		...
		if decl.(*ast.File).Decls != nil {
			for _, d := range decl.(*ast.File).Decls {
				SearchNodes(d, foundNodes, parents, searchFunction, completed)
			}
		}
		if decl.(*ast.File).Imports != nil {
			for _, spec := range decl.(*ast.File).Imports {
				SearchNodes(spec, foundNodes, parents, searchFunction, completed)
			}
		}
		if decl.(*ast.File).Unresolved != nil {
			for _, ident := range decl.(*ast.File).Unresolved {
				SearchNodes(ident, foundNodes, parents, searchFunction, completed)
			}
		}
		if decl.(*ast.File).Comments != nil {
			for _, comment := range decl.(*ast.File).Comments {
				SearchNodes(comment, foundNodes, parents, searchFunction, completed)
			}
		}
	}
}
